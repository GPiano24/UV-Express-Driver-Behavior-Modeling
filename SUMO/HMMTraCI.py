...
def load_annotated_data(file_name):
...
def calculate_start_probabilities(observed_states):
...
def get_transition_states(observed_state_changes):
    temp = ''
    transition_state = []
...
def calculate_transition_probabilities(transition_state):
...
# Get emission states
def get_emission_states(observed_states, observed_events, total_count):
...
def calculate_emission_probabilities(emission):
